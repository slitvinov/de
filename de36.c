...
double (*pold)[MAXPOP][MAXDIM], (*pnew)[MAXPOP][MAXDIM],
    (*pswap)[MAXPOP][MAXDIM];
...

{
  ...
  fscanf(fpin_ptr, "%lf",
         &inibound_h); /*---upper parameter bound for init-----*/
  fscanf(fpin_ptr, "%lf",
  ...
  {                                            /* is accepted by compiler    */
    ...
    for (i = 0; i < NP; i++) /* Start of loop through ensemble  */
    {
      ...
      if (NP >= 6) {
        ...
        } while ((r5 == i) || (r5 == r1) || (r5 == r2) || (r5 == r3) ||
                 (r5 == r4));
      }

      /*=======Choice of
       * strategy===============================================================*/
      /*=======We have tried to come up with a sensible naming-convention:
       * DE/x/y/z=============*/
      /*=======DE :  stands for Differential
       * Evolution==========================================*/
      /*=======x  :  a string which denotes the vector to be
       * perturbed==========================*/
      /*=======y  :  number of difference vectors taken for perturbation of
       * x===================*/
      /*=======z  :  crossover method (exp = exponential, bin =
       * binomial)=======================*/
      /*                                                                                        */
      /*=======There are some simple rules which are worth
       * following:===========================*/
      /*=======1)  F is usually between 0.5 and 1 (in rare cases >
       * 1)===========================*/
      /*=======2)  CR is between 0 and 1 with 0., 0.3, 0.7 and 1. being worth to
       * be tried first=*/
      /*=======3)  To start off NP = 10*D is a reasonable choice. Increase NP if
       * misconvergence=*/
      /*           happens. */
      /*=======4)  If you increase NP, F usually has to be
       * decreased============================*/
      ...
      /*-------DE/best/1/exp--------------------------------------------------------------------*/
      /*-------Our oldest strategy but still not bad. However, we have found
       * several------------*/
      /*-------optimization problems where misconvergence
       * occurs.-------------------------------*/
      ...
      /*-------DE/rand/1/exp-------------------------------------------------------------------*/
      /*-------This is one of my favourite strategies. It works especially well
         when the-------*/
      /*-------"bestit[]"-schemes experience misconvergence. Try e.g. F=0.7 and
         CR=0.5---------*/
      /*-------as a first
         guess.---------------------------------------------------------------*/
      ...
      /*-------DE/rand-to-best/1/exp-----------------------------------------------------------*/
      /*-------This strategy seems to be one of the best strategies. Try F=0.85
         and CR=1.------*/
      /*-------If you get misconvergence try to increase NP. If this doesn't
         help you----------*/
      /*-------should play around with all three control
         variables.----------------------------*/
      else if (strategy == 3) /* similiar to DE2 but generally better */
      {
        ...
        L = 0;
        do {
          tmp[n] = tmp[n] + F * (bestit[n] - tmp[n]) +
                   F * ((*pold)[r1][n] - (*pold)[r2][n]);
          n = (n + 1) % D;
          L++;
        } while ((rnd_uni(&rnd_uni_init) < CR) && (L < D));
      }
      /*-------DE/best/2/exp is another powerful strategy worth
         trying--------------------------*/
      else if (strategy == 4) {
        assignd(D, tmp, (*pold)[i]);
        n = (int)(rnd_uni(&rnd_uni_init) * D);
        L = 0;
        do {
          tmp[n] = bestit[n] + ((*pold)[r1][n] + (*pold)[r2][n] -
                                (*pold)[r3][n] - (*pold)[r4][n]) *
                                   F;
          n = (n + 1) % D;
          L++;
      ...
      /*-------DE/rand/2/exp seems to be a robust optimizer for many
        ...
        L = 0;
        do {
          tmp[n] = (*pold)[r5][n] + ((*pold)[r1][n] + (*pold)[r2][n] -
                                     (*pold)[r3][n] - (*pold)[r4][n]) *
                                        F;
          n = (n + 1) % D;
          L++;
      ...
      /*=======Essentially same strategies but BINOMIAL
         CROSSOVER===============================*/
      ...
      /*-------DE/best/1/bin--------------------------------------------------------------------*/
        ...
        {
          if ((rnd_uni(&rnd_uni_init) < CR) ||
              L == (D - 1)) /* change at least one parameter */
          {
            tmp[n] = bestit[n] + F * ((*pold)[r2][n] - (*pold)[r3][n]);
          }
          n = (n + 1) % D;
      ...
      /*=======Trial mutation now in tmp[]. Test how good this choice really
       * was.==================*/
    ...
    assignd(D, bestit,
            best); /* Save best population member of current iteration */
    ...
    /*----Compute the energy variance (just for monitoring
     * purposes)-----------*/
  ...
  fprintf(fpout_ptr, "\n\n Generation=%d  NFEs=%ld   Strategy: %s    ", gen,
          nfeval, strat[strategy]);
  fprintf(fpout_ptr,
          "\n NP=%d    F=%-4.2g    CR=%-4.2g    cost-variance=%-10.5g\n", NP, F,
          CR, cvar);
